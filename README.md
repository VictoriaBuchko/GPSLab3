...
Після запуску емулятор доступний за адресою `ws://localhost:4001` (WebSocket) та `http://localhost:4001/config` (REST API для зміни параметрів).
...
Додаток підключається до `ws://localhost:4001` одразу при завантаженні. Кожне повідомлення парситься з JSON і зберігається у `Map` за ідентифікатором супутника. При втраті з'єднання відбувається автоматичне перепідключення через 2 секунди:
...
Для розрахунку обираються 3 супутники з найменшою затримкою сигналу:
...
**Аналітичний метод** - лінеаризація системи трьох рівнянь кіл. З пар рівнянь s2-s1 та s3-s2 отримується лінійна система 2x2, яка розв'язується через визначник:
...
**Чисельний метод** - власна реалізація градієнтного спуску. Мінімізується сума квадратів різниць між виміряними та розрахованими відстанями до супутників. Початкове наближення - центр мас трьох супутників:
...
Графік побудовано на Chart.js (scatter plot). Три датасети оновлюються після кожного нового повідомлення. Якщо супутник виходить за межі зони - розрахунок призупиняється, попередні позиції залишаються на графіку:
...
![Попередження про вихід за межі](images/interface_warning.png)
Рис. 4 - Статус при виході супутника за межі зони
...
| `connectWS()` | підключення до WebSocket з автоперепідключенням |
...
У ході виконання роботи розроблено веб-додаток для візуалізації GPS-вимірювань, що працює повністю на клієнтській стороні без бекенду. Реалізовано два незалежні методи розрахунку координат: аналітичний дає точний результат для ідеальних даних з похибкою близькою до нуля, тоді як чисельний метод на основі власного градієнтного спуску сходиться до схожого результату за 200 ітерацій і є стійкішим до шумів у реальних вимірах.
